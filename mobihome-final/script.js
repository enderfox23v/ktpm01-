...
// ================= GLOBAL LEAD CAPTURE FORM (GMAIL) =================
window.handleLeadSubmit = function(e) {
    ...
    if (!leadEmailInput || !leadForm || !locationsResult) return false;
    ...
    if (emailVal.length >= 1) {
        ...
        locationsResult.style.display = 'block';
        locationsResult.scrollIntoView({ behavior: 'smooth', block: 'center' });
    } else {
        if (emailError) emailError.style.display = 'block';
        leadEmailInput.style.border = '1px solid #ef4444';
    }
    return false;
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (leadEmailInput) {
        leadEmailInput.addEventListener('input', () => {
            if (emailError) emailError.style.display = 'none';
            leadEmailInput.style.border = 'none';
        });
    }
});
